...
private[kafka] object ConsumerStage {
  def plainSubSource[K, V](settings: ConsumerSettings[K, V], subscription: AutoSubscription, loadOffsetOnAssign: Option[TopicPartition => Long] = None, onRevoke: TopicPartition => Unit = _ => ()) = {
