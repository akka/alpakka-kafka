...
class CommittingSpec extends SpecBase with TestcontainersKafkaLike with Inside {
  ...
  "Committing" must {
    ...
    "retry commits that occurred during rebalance" in assertAllStagesStopped {
      val count = 10
      val topic1 = createTopic(1, partitions = 2)
      val group1 = createGroupId(1)
      val consumerSettings = consumerDefaults
        .withGroupId(group1)
        .withProperty("partition.assignment.strategy", "org.apache.kafka.clients.consumer.CooperativeStickyAssignor")
